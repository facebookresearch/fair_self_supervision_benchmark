...
def create_model(model, data, labels, split):
    ...
    if not cfg.MODEL.EXTRACT_FEATURES_ONLY:
        ...
        if split == 'train':
            ...
            softmax = model.Softmax('pred_conv1', 'softmax_conv1', engine='CUDNN')
            loss_c1 = None
    ...
    if not cfg.MODEL.EXTRACT_FEATURES_ONLY:
        ...
        if split == 'train':
            softmax, loss_c2 = model.SoftmaxWithLoss(
                ['pred_conv2', labels],
                ['softmax_conv2', 'loss_conv2'], scale=scale
            )
        elif split in ['test', 'val']:
            softmax = model.Softmax('pred_conv2', 'softmax_conv2', engine='CUDNN')
            loss_c2 = None
    ...
    if not cfg.MODEL.EXTRACT_FEATURES_ONLY:
        ...
        if split == 'train':
            ...
            softmax = model.Softmax('pred_conv3', 'softmax_conv3', engine='CUDNN')
            loss_c3 = None
    ...
    if not cfg.MODEL.EXTRACT_FEATURES_ONLY:
        ...
        if split == 'train':
            ...
            softmax = model.Softmax('pred_conv4', 'softmax_conv4', engine='CUDNN')
            loss_c4 = None
    ...
    resize_c5 = model.AveragePool(
        blob_in, blob_in + '_s1k6_resize', stride=1, kernel=6, pad=0
    )
    bn_c5 = model.SpatialBN(
        resize_c5, resize_c5 + '_bn', num_features,
    ...
    fc_conv5 = model.FC(
        bn_c5, 'conv5_cls', num_features * 2 * 2, num_classes,
        weight_init=('GaussianFill', {'std': 0.01}),
        bias_init=('ConstantFill', {'value': 0.0}),
    )
    ...
    if not cfg.MODEL.EXTRACT_FEATURES_ONLY:
        ...
        if split == 'train':
            softmax, loss_c5 = model.SoftmaxWithLoss(
                [fc_conv5, labels],
                ['softmax_conv5', 'loss_conv5'],
                scale=scale
            )
        elif split in ['test', 'val']:
            softmax = model.Softmax('pred_conv5', 'softmax_conv5', engine='CUDNN')
            loss_c5 = None
