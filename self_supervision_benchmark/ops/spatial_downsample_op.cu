...
#define CALL_K(X) else if (MR == X) tentx_k<X><<<dim3((W-1)/BX+1, (H-1)/BY+1, D), dim3(BX,BY,1), NS, context->cuda_stream()>>>(in, W, H, D, out, R)
  CALL_K(1); CALL_K(2); CALL_K(3); CALL_K(4); CALL_K(5); CALL_K(6); CALL_K(7);
  CALL_K(8); CALL_K(9); CALL_K(10);
  else {
...
template <>
bool SpatialDownsampleOp<float, CUDAContext>::RunOnDevice() {
  ...
  int tH = int(floor(H * scale_y_));
  int tW = int(floor(W * scale_x_));
;
  // tmp0->ResizeLike(X);
  // tmp1->ResizeLike(X);
  // top->Resize(std::vector<int>{N, C, tH, tW});

  auto* top = Output(0, {N, C, tH, tW}, at::dtype<float>());
  auto* tmp0 = Output(1, X.sizes(), at::dtype<float>());
  auto* tmp1 = Output(2, X.sizes(), at::dtype<float>());
...
template <>
bool SpatialDownsampleGradientOp<float, CUDAContext>::RunOnDevice() {
  auto& X = Input(0);     // original
  auto& top = Input(1);   // resized output
  auto& tmp0 = Input(2);   // resized output
  auto& tmp1 = Input(3);   // resized output
  auto& dTop = Input(4);  // resized output
  // auto* dX = Output(0);
  ...
  context_.CopySameDevice<float>(X.size(), tmp0Data, p_tmp0);
  context_.CopySameDevice<float>(X.size(), tmp1Data, p_tmp1);
...
REGISTER_CUDA_OPERATOR(SpatialDownsample,
                       SpatialDownsampleOp<float, CUDAContext>);
REGISTER_CUDA_OPERATOR(SpatialDownsampleGradient,
                       SpatialDownsampleGradientOp<float, CUDAContext>);
} // namespace caffe2
